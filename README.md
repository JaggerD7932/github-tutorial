...
- Type in `mkdir FILENAME` to create a new direcotry
- Then `cd FILENAME` you will be putting in the name of the directory you had just created  
...
- Now that you've initialized, create a README.md file by simply `touch README.md` and add something to it.  
...
- To do this, type in `git add <filename>`, with `<filename>` in this case is the README.md file
...
A bad commit message, on the other hand, might be "initialized and committed", as it is the opposite.
...
3) You'll want to `git add` your changes to the stage, so go ahead and do that
